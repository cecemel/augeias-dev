...
def build_docker_modules(git_user, git_pw, modules=[]):
    ...
    for folder in docker_files_folders:
            ...
            docker_files_root_dir = os.path.join(docker_files_dir, folder)
            sub_docker_files_folders = _listdir_not_hidden(docker_files_root_dir)

            # check if any subfolders (for e.g. multiple images per repo, e.g. augeias)
            if not sub_docker_files_folders:
                _build_image(git_user, git_pw, current_path, docker_files_dir, folder)
                continue

            for sub_folder in sub_docker_files_folders:
                _build_image(git_user, git_pw,
                    os.path.join(current_path, folder), docker_files_root_dir, sub_folder)
                os.chdir(current_path)


def _build_image(git_user, git_pw, current_path, docker_files_dir, folder):
    try:
